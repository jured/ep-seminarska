...
class DB
{
    ...
    private $_SALT = "tojeenagroznasol";
    ...
    public function user_register($user)
    {
        ...
        if (!isset($user['email']) || !isset($user['name'])|| !isset($user['lastname']) || !isset($user['password'])) {
            error_log("Missing email and password!");
            return false;
        }
    ...
    public function user_login($email, $password)
    {
        ...
        if ($this->get_user_mail->execute(array('email' => $email))) {
            ...
            error_log("Trying to login user: " . json_encode($user));

            if ($user && $user['active'] && $password != null && ($this->hashPassword($password)) == $user['password']) {
